...
class KerrArray(np.ndarray):
    ...
    for mod in _ski_modules: 
        _ski_funcs[mod.__name__] = dir(mod)
    
    #now initialise class
    
    def __new__(cls, image, metadata={}, get_metadata=True, floatdata=True):
        """
        ...
        floatdata: bool
            converts the image to float 0.0-1.0 (higher resolution than uint8 
            saved by Evico)
        Returns
        -------
        ka: KerrArray
    ...
    def __init__(self, image, metadata={}, get_metadata=True, floatdata=True):
...
#==============================================================    
    def __getattr__(self,name):
        ...
        Also note that the function named must take image array is the 
        first argument"""
        ...
        if name in dir(kfuncs):
            ...
            for key in skifuncs.keys():
                if name in skifuncs[key]:
                    workingfunc=getattr(key,name)
                    break
            ret=self._func_generator(workingfunc)
        if ret is None:
            raise ValueError('No attribute found of name {}'.format(name))
        return ret
...
#==============================================================  
    def box(self, xmin, xmax, ymin, ymax):
        ...
        [[2,3,3],[1,2,2]]
        """
        return self.crop_image(box=box,copy=False)
